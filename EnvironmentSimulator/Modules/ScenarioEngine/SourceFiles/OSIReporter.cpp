...
int OSIReporter::UpdateOSIIntersection()
{
	...
	for (int i=0; i<opendrive->GetNumOfJunctions(); i++)
	{
		...
		if (junction->IsOsiIntersection())
		{
			...
			for (int i=0;i<left_lane_lengths.size();i++)
			{
				...
				{
					int same_left = roadmanager::CheckOverlapingOSIPoints(left_lane_lengths[i].osipoints,lane_lengths[j].osipoints,tolerance);
					if (same_left >0) 
					{
						keep_left = false;
					}
					int same_right = roadmanager::CheckOverlapingOSIPoints(right_lane_lengths[i].osipoints,lane_lengths[j].osipoints,tolerance);
					if (same_right >0)
					{
						keep_right = false;
					}
				...
				if (keep_left) 
				{
					tmp_lane_lengths.push_back(left_lane_lengths[i]);
				}
				else if (keep_right)
				{
					tmp_lane_lengths.push_back(right_lane_lengths[i]);
				}
			}
			...
			else
			{
				...
				for (int j=0;j<lane_lengths.size();j++)
				{
					...
					{
						for (int i=0;i<lane_lengths.size();i++)
						{
							if (i!=j)
							{
