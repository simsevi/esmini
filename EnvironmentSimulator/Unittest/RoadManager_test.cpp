...
TEST_P(OSIPointsCloseCheck, ClosenessChecker) {
    ...
    OSIPoints::OSIPointStruct s1;
    s1.x = std::get<0>(GetParam());
    s1.y = std::get<1>(GetParam());

    OSIPoints::OSIPointStruct s2;
    s2.x = std::get<2>(GetParam());
    s2.y = std::get<3>(GetParam());

    OSIPoints::OSIPointStruct s3;
    s3.x = std::get<4>(GetParam());
    s3.y = std::get<5>(GetParam());

    OSIPoints::OSIPointStruct s4;
    s4.x = std::get<6>(GetParam());
    s4.y = std::get<7>(GetParam());

    std::vector<OSIPoints::OSIPointStruct> vec1;
    vec1.push_back(s1);
    vec1.push_back(s2);

    std::vector<OSIPoints::OSIPointStruct> vec2;
    vec2.push_back(s3);
    vec2.push_back(s4);
